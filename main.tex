...
\begin{proof}
For each $i=1,\dots,k$, let $r_i=\rank(A_{ii})$.  
Choose $r_i$ linearly independent columns from the block $A_{ii}$.

Now consider in $A$ the full columns obtained by taking these chosen columns from each diagonal block.  
We claim that all these columns are linearly independent in $A$.

Indeed, suppose a linear combination of them is zero. Look at the first block of rows.  
Since all blocks to the right of $A_{11}$ in the first block row are zero, this gives a linear relation among the chosen columns of $A_{11}$.  
But those columns are linearly independent, so all coefficients corresponding to $A_{11}$ must be zero.

Next, look at the second block of rows. After the coefficients from the first block are already known to be zero, we obtain a linear relation among the chosen columns of $A_{22}$.  
Again, by independence of those columns, all coefficients corresponding to $A_{22}$ must be zero.

Continuing in this way block by block, we conclude that all coefficients are zero.  
Hence the selected columns are linearly independent in $A$.

Therefore,
\[
\rank(A)\ge r_1+\cdots+r_k
=\rank(A_{11})+\cdots+\rank(A_{kk}).
\]

Now suppose in addition that $A_{ij}=0$ for all $i>j$. Since also $A_{ij}=0$ for all $i<j$ by the given form of $A$, the matrix $A$ is block diagonal:
\[
A=\operatorname{diag}(A_{11},A_{22},\dots,A_{kk}).
\]
For a block diagonal matrix, the column space is the direct sum of the column spaces of the diagonal blocks. Hence
\[
\rank(A)=\rank(A_{11})+\cdots+\rank(A_{kk}).
\]
...
\begin{exercise}
Let $A$ be an orthogonal $n\times n$ matrix. Show that $a_{11}$ and $\det A(1\mid 1)$ have the
same absolute value.
\end{exercise}
\begin{proof}
If $A$ is orthogonal then $A^\top A=AA^\top =I$. Thus $A^{-1} = A^\top$, so
\[
\det(A^{-1})=\det(A^\top)=\det(A)
\]
and hence
\[
\frac{1}{\det(A)}=\det(A)\ \Longrightarrow\ (\det(A))^2=1\ \Longrightarrow\ \det(A)=\pm 1,
\]
so $|\det(A)|=1$.
Now use
\[
...
\quad\text{and}\quad
(\adj(A))_{i,j}=(-1)^{i+j}\det(A(j\mid i)).
\]
Taking $(1,1)$-entries and using $A^{-1}=A^\top$ gives
\begin{align*}
a_{11}=(A^\top)_{11}=(A^{-1})_{11}
&=\frac{(\adj(A))_{11}}{\det(A)}
=\frac{(-1)^{1+1}\det(A(1\mid 1))}{\det(A)}\\
&=\frac{\det(A(1\mid 1))}{\det(A)}.
\end{align*}
Taking absolute values,
\[
|a_{11}|=\frac{|\det(A(1\mid 1))|}{|\det(A)|}=|\det(A(1\mid 1))|.
\]
\end{proof}
...
\begin{proof}
Recall that $G=A^{+}$ if and only if the four Penrose equations hold:
...
(GA)^\top=GA.
\end{equation}
We prove that these are equivalent to
...
Assume $G=A^+$. From \eqref{eq:p1},
\[
A^\top=(AGA)^\top=A^\top(AG)^\top.
\]
Using \eqref{eq:p3}, we get
\[
A^\top=A^\top AG,
\]
which is \eqref{eq:c1}.
Similarly, from \eqref{eq:p2},
\[
G^\top=(GAG)^\top=G^\top(GA)^\top.
\]
Using \eqref{eq:p4}, we obtain
\[
G^\top=G^\top GA,
\]
which is \eqref{eq:c2}.

\medskip
\noindent
\textbf{Backward direction.}
Assume that \eqref{eq:c1} and \eqref{eq:c2} hold. We show that the four Penrose equations follow.
First,
\[
(AG)^\top=G^\top A^\top.
\]
...
(GA)^\top=GA.
\]
Thus \eqref{eq:p4} holds.
Now, from \eqref{eq:c1} and \eqref{eq:p3},
\[
A^\top=A^\top AG=A^\top(AG)^\top=A^\top G^\top A^\top.
\]
Taking transpose, we obtain
\[
A=AGA.
\]
So \eqref{eq:p1} holds.
Likewise, from \eqref{eq:c2} and \eqref{eq:p4},
\[
G^\top=G^\top GA=G^\top (GA)^\top=G^\top A^\top G^\top.
\]
Taking transpose, we obtain
\[
G=GAG.
\]
So \eqref{eq:p2} holds.
Therefore all four Penrose equations hold, and hence $G=A^+$.
This proves that
\[
G=A^+
